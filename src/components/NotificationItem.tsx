...
export const NotificationItem = ({ notification: { id, sender, issue, reason, alreadyRead, comment, project, pullRequest} }: Props) => {
  const currentSpace = useCurrentSpace()

  const accessories: List.Item.Accessory[] = [
    { icon: { source: `https://${currentSpace.host}/api/v2/users/${sender.id}/icon?apiKey=${currentSpace.apiKey}`, mask: Image.Mask.Circle}, tooltip: sender.name },
  ]
  const tintColor = alreadyRead ? Color.SecondaryText : Color.Orange
  const url = pullRequest ? `https://${currentSpace.host}/git/${project.projectKey}/app/pullRequests/${pullRequest.number}`
  : issue ? comment ? `https://${currentSpace.host}/view/${issue.issueKey}#comment-${comment.id}` : `https://${currentSpace.host}/view/${issue.issueKey}`
